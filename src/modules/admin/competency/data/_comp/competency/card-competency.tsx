...
export function CardCompetency() {
    ...
    const { data, loading, category, proficiency, addData, updateData, deleteData } = useLogic();
    ...
    return (
        <>
                                ...
                                loading && data?.length === 0 ? (
                                    ...
                                    {data?.map((i: any, k: number) => (
                                        <TableRow key={k}>
                                            ...
                                            <TableCell>
                                                <Button variant="outline" onClick={() => setOpenView({ data: i?.proficiency, category: category, isOpen: !openView.isOpen })} size="sm"><IconEye /> View</Button>
                                            </TableCell>
                                            <TableCell>
                                                {
            ...
            <AddDialog openAdd={openAdd} category={category} proficiency={proficiency} setOpenAdd={setOpenAdd} addData={addData} />
            <UpdateDialog openUpdate={openUpdate.isOpen} setOpenUpdate={() => setOpenUpdate({ ...openUpdate, isOpen: !openUpdate.isOpen })} onUpdateData={updateData} updateData={openUpdate?.data} category={category} proficiency={proficiency} />
            <DeleteDialog openDelete={openDelete.isOpen} setOpenDelete={setOpenDelete} onDeleteData={deleteData} deleteData={openDelete?.data} />
            <ViewDialog openView={openView} setOpenView={() => setOpenView({ data: openView.data, category: openView.category, isOpen: !openView.isOpen })} />
        </>
    );
}
