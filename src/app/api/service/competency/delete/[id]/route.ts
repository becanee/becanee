...
/**
 * Func: DELETE endpoint untuk menghapus data competency dari API server
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        const { data: DeleteCompetency, error: ErrorDeleteCompetency } = await supabase
            .from('m_competency')
            .delete()
            .eq('id', id)
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data competency berhasil dihapus",
            powered_by: "PT Klola Indonesia",
            data: DeleteCompetency?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menghapus data competency",
                powered_by: "PT Klola Indonesia",
                data: null,
