...
/**
 * Func: DELETE endpoint untuk menghapus data competency category dari API server
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        const { data: DeleteCompetencyCategory, error: ErrorDeleteCompetencyCategory } = await supabase
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data category competency berhasil diupdate",
            powered_by: "PT Klola Indonesia",
            data: DeleteCompetencyCategory?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menghapus data category competency",
                powered_by: "PT Klola Indonesia",
                data: null,
