...
export async function GET(request: NextRequest) {
    try {
        ...
        let { data: GetCompetencyCategory, error: GetCompetencyCategoryError }: any = await supabase
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data category competency berhasil diambil",
            powered_by: "PT Klola Indonesia",
            data: GetCompetencyCategory,
            pagination: {
                link: {
                    first: request.url,
                    next: Math.ceil(GetCompetencyCategory.length / 10) > 1 ? request.url : null,
                    last: request.url
                },
                total: GetCompetencyCategory.length,
                per_page: 10,
                current_page: 1,
                total_pages: Math.ceil(GetCompetencyCategory.length / 10)
            }
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal mengambil data category competency",
                powered_by: "PT Klola Indonesia",
                data: null,
