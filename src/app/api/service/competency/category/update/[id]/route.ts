...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { data: UpdateCompetencyCategory, error: ErrorUpdateCompetencyCategory } = await supabase
        ...
        return NextResponse.json({
            ...
            powered_by: "PT Klola Indonesia",
            data: UpdateCompetencyCategory?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal mengupdate data category competency",
                powered_by: "PT Klola Indonesia",
                data: null,
