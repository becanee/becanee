...
/**
 * Func: DELETE endpoint untuk menghapus data mcqs dari API server
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        const { data: DeleteMcqs, error: ErrorDeleteMcqs } = await supabase
            .from('m_mcqs')
            .delete()
            .eq('id', id)
            .select('*')
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data mcqs berhasil dihapus",
            powered_by: "PT Klola Indonesia",
            data: DeleteMcqs?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menghapus data mcqs",
                powered_by: "PT Klola Indonesia",
                data: null,
