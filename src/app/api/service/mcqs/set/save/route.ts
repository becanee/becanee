...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: InsertMcqsSet, error: ErrorInsertMcqsSet } = await supabase
            .from('m_mcqs_set')
            .insert([
                {
                    created_by,
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data mcqs set berhasil disimpan",
            powered_by: "PT Klola Indonesia",
            data: InsertMcqsSet?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menyimpan data mcqs set",
                powered_by: "PT Klola Indonesia",
                data: null,
