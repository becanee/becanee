...
/**
 * Func: GET endpoint untuk mengambil data proficiency dari API server
...
export async function GET(request: NextRequest) {
    try {
        ...
        let { data: GetProficiency, error: GetProficiencyError }: any = await supabase
            .from('m_proficiency')
            .select("*, level(*)")
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data proficiency berhasil diambil",
            powered_by: "PT Klola Indonesia",
            data: GetProficiency,
            pagination: {
                link: {
                    first: request.url,
                    next: Math.ceil(GetProficiency.length / 10) > 1 ? request.url : null,
                    last: request.url
                },
                total: GetProficiency.length,
                per_page: 10,
                current_page: 1,
                total_pages: Math.ceil(GetProficiency.length / 10)
            }
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal mengambil data proficiency",
                powered_by: "PT Klola Indonesia",
                data: null,
