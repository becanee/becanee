...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: InserProficiency, error: ErrorInserProficiency } = await supabase
            .from('m_proficiency')
            .insert([
                {
                    created_by,
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data proficiency berhasil disimpan",
            powered_by: "PT Klola Indonesia",
            data: InserProficiency?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menyimpan data proficiency",
                powered_by: "PT Klola Indonesia",
                data: null,
