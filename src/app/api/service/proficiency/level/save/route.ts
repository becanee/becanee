...
export async function POST(request: NextRequest) {
    try {
        ...
        const { data: InserProficiencyLevel, error: ErrorInserProficiencyLevel } = await supabase
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data proficiency level berhasil disimpan",
            powered_by: "PT Klola Indonesia",
            data: InserProficiencyLevel?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menyimpan data proficiency level",
                powered_by: "PT Klola Indonesia",
                data: null,
