...
/**
 * Func: DELETE endpoint untuk menghapus data proficiency level dari API server
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        const { data: DeleteProficiencyLevel, error: ErrorDeleteProficiencyLevel } = await supabase
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data proficiency level berhasil dihapus",
            powered_by: "PT Klola Indonesia",
            data: DeleteProficiencyLevel?.[0],
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal menghapus data proficiency level",
                powered_by: "PT Klola Indonesia",
                data: null,
