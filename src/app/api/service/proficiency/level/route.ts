...
/**
 * Func: GET endpoint untuk mengambil data proficiency level dari API server
...
export async function GET(request: NextRequest) {
    try {
        ...
        let { data: GetProficiencyLevel, error: GetProficiencyLevelError }: any = await supabase
        ...
        return NextResponse.json({
            status: true,
            code: 200,
            message: "Data proficiency level berhasil diambil",
            powered_by: "PT Klola Indonesia",
            data: GetProficiencyLevel,
            pagination: {
                link: {
                    first: request.url,
                    next: Math.ceil(GetProficiencyLevel.length / 10) > 1 ? request.url : null,
                    last: request.url
                },
                total: GetProficiencyLevel.length,
                per_page: 10,
                current_page: 1,
                total_pages: Math.ceil(GetProficiencyLevel.length / 10)
            }
        });
        ...
        if (error.response) {
            return NextResponse.json({
                ...
                message: error.response.data?.message || "Gagal mengambil data proficiency level",
                powered_by: "PT Klola Indonesia",
                data: null,
